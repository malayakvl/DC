...
import { DoorOpen, Check, CalendarRange } from 'lucide-react';

export default function NavScheduler() {
            ...
            <div>
              <div className="p-2 space-y-1">
                <div className="px-3 py-1.5 text-[10px] font-bold tracking-wider uppercase text-slate-400">
                  {lng.get('menu.tipStore.scheduler_section') || 'Розклад та кабінети'}
                </div>
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-slate-500 line-clamp-1 mt-0.5">
                          {lng.get('menu.tipStore.cabinets') || 'Управління кабінетами'}
                        </p>
                      </div>
                    </Link>
                  </MenuItem>
                ...
                {permissions['patient-edit'] && (
                  <MenuItem>
                    <Link
                      href={'/visit-schedule-statuses'}
                      className="group flex items-start gap-3 p-2.5 rounded-xl hover:bg-teal-50/70 transition-colors"
                    >
                      <div
                        className={`mt-0.5 w-9 h-9 rounded-lg flex items-center justify-center shrink-0 group-hover:scale-105 group-hover:bg-teal-600 group-hover:text-white transition-all shadow-xs ${
                          url.startsWith('/visit-schedule-statuses')
                            ? 'bg-teal-600 text-white'
                            : 'bg-slate-100 text-slate-600'
                        }`}
                      >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-slate-500 line-clamp-1 mt-0.5">
                          {lng.get('menu.tipStore.visit_schedule_statuses')}
                        </p>
                      </div>
                    </Link>
                  </MenuItem>
                  ...
                  <MenuItem>
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-slate-500 line-clamp-1 mt-0.5">
                          {lng.get('menu.tipStore.scheduler') || 'Графік та календар візитів'}
                        </p>
                      </div>
                    </Link>
                  </MenuItem>
