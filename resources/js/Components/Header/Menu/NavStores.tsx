...
export default function NavStores() {
  ...
  // 1. Список всех путей, относящихся к этому меню
  const activeRoutes = ['/units', '/producers', '/suppliers', '/material-categories', '/materials'];

  // 2. Проверяем, начинается ли текущий URL с какого-либо из путей
    ...
    <>
      {(props.auth.user?.roles[0]?.name === 'Admin' || permissions['store-all']) && (
        <Menu as="div" className="relative top-menu-nav">
          <MenuButton
            /* 3. Добавляем класс активности (например, active или text-blue-600) */
            className={`top-nav flex flex-col items-center ${isActive ? 'active text-blue-600' : ''}`}
          >
          ...
          >
            <div>
              {permissions['store-create'] && (
                <MenuItem>
                  <Link
                    className={`submenu ${url.startsWith('/units') ? 'font-bold text-blue-600' : ''}`}
                    href={'/units'}
                  >
              ...
              {permissions['store-create'] && (
                <MenuItem>
                  <Link
                    className={`submenu ${url.startsWith('/producers') ? 'font-bold text-blue-600' : ''}`}
                    href={'/producers'}
                  >
              ...
              {permissions['store-create'] && (
                <MenuItem>
                  <Link
                    className={`submenu ${url.startsWith('/suppliers') ? 'font-bold text-blue-600' : ''}`}
                    href={'/suppliers'}
                  >
              ...
              {permissions['store-create'] && (
                <MenuItem>
                  <Link
                    className={`submenu ${url.startsWith('/material-categories') ? 'font-bold text-blue-600' : ''}`}
              ...
              {permissions['store-create'] && (
                <MenuItem>
                  <Link
                    className={`submenu ${url.startsWith('/materials') ? 'font-bold text-blue-600' : ''}`}
                    href={'/materials'}
                  >
