...
export default function SchedulerFormEdit({
  ...
  return (
    <section
      className={`px-5 max-h-[75vh] form-scheduler form-edit-schedulter bg-white overflow-y-auto ${showPopup ? '' : 'hidden'}`}
    >
      <header>
        <h2 className={'pt-7 pb-7'}>{msg.get('scheduler.title.edit.visit')}!</h2>
      </header>
      ...
      >
        <EventStatus
          defaultStatus={editEventData.event_status}
          defaultColor={editEventData.status_color}
        />
        ...
        <div className={`relative`}>
          <label htmlFor="title" className="block text-sm font-medium text-gray-700 ">
            ...
            <span className="text-discount float-right">
              &nbsp;
              {editEventData.status_name &&
                `${editEventData.patient_status_name} (-${editEventData.discount}%)`}
            </span>
          </label>
          <div className={'sh-p-view'}>
            {editEventData.pl_name} {editEventData.p_name}
          </div>
        </div>
