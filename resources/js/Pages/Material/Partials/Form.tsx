...
export default function Form({
  ...
  const searchUnitResults = useSelector(unitSearchResultsSelector);
  ...
  useEffect(() => {
    if (data.price && categoryPercent) {
      ...
      setData((prev) => ({
        ...
        price_per_unit: (
          parseFloat(retailValue) / parseFloat(prev.weight ? (prev.weight as string) : '1')
        ).toFixed(2),
      }));
    } else {
          ...
          <div className="w-2/3">
            ...
            <div className="relative">
              ...
              <span className="percent-price">
                {categoryPercent && msg.get('material.percent') + ': ' + categoryPercent + '%'}
              </span>
            </div>
            <InputText
              name={'retail_price'}
