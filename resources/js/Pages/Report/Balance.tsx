...
export default function Balance({ filials, dateFrom, dateTo, customerData }: BalanceProps) {
  ...
  const generateReport = () => {
    ...
    axios.post(`/report/generateBalanceReport`, { values }, {}).then((res) => {
                ...
                <React.Fragment key={index}>
                  {isNewPatient && (
                    <tr
                      style={{
                        background:
                          finalBalance < 0 ? '#610b0b' : finalBalance > 0 ? '#f67d1a' : '#427245',
                        fontWeight: 600,
                        color: '#fff',
                      }}
                    >
                  ...
                  <tr>
                    ...
                    <td style={td}>{new Date(item.document_date).toLocaleDateString()}</td>

                    <td style={td}>
                      {isAct ? '🧾 Act' : '💳 Payment'} <strong>{item.document_number}</strong>
                    </td>
