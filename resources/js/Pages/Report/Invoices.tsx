...
export default function Invoices({ filials, dateFrom, dateTo, suppliers }: BalanceProps) {
  ...
  const generateReport = () => {
    ...
    axios
      .post(`/report/generateInvoicesReport`, { values: currentValues }, {})
      .then((res) => {
          ...
          <thead>
            <tr style={{ background: '#1f2937' }}>
              ...
              <th style={{ ...thStyle, textAlign: 'center', width: '15%' }}>
                {msg.get('report.title.opening_balance')}
              </th>
              <th style={{ ...thStyle, textAlign: 'center', width: '15%' }}>
                {msg.get('report.title.income')}
              </th>
              <th style={{ ...thStyle, textAlign: 'center', width: '15%' }}>
                {msg.get('report.title.expense')}
              </th>
              <th style={{ ...thStyle, textAlign: 'right', width: '15%' }}>
                {msg.get('report.title.closing_balance')}
              </th>
            </tr>
          </thead>
          <tbody>
                  ...
                  return (
                    <tr key={mIdx}>
                      ...
                      <td style={{ ...tdStyle }}>
                        ...
                        >
                          {item.document_type
                            ...
                            : ''}
                          {item.document_id
                            ? (item.document_type === 'payment' ? ' за накладною ' : '') +
                              ' #' +
                              item.document_id +
                              ''
                            : ''}
                      ...
                      <td style={{ ...tdStyle, textAlign: 'center' }}>
                        {Number(item.debit) > 0 ? (
                          <span style={{ color: '#4ade80' }}>{fmtNum(item.debit)}</span>
                        ) : (
                          ''
                        )}
                      </td>
                      <td style={{ ...tdStyle, textAlign: 'center' }}>
                        {Number(item.credit) > 0 ? (
                          <span style={{ color: '#f178d7ff' }}>{fmtNum(item.credit)}</span>
                        ) : (
                          ''
                        )}
                      </td>
                      <td
                        style={{
                          ...tdRightStyle,
                          color: Number(item.running_balance) > 0 ? '#f178d7ff' : '#9ca3af',
                        }}
                      >
              ...
              <header>
                <div className="flex inline-flex">
                  <h2 className="text-white" style={{ fontSize: '1rem', color: 'white' }}>
                    {msg.get('report.title.suppliers.report')}
                  </h2>
