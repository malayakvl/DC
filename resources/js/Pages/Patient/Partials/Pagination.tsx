...
export default function Pagination({ listData }) {
  ...
  return (
    <div className="mt-4 flex justify-between items-center mt-2 mb-2">
      <div>
        <p className="pagination-title">
          {msgLayouts.get('layouts.pagination.show')} {listData.current_page}{' '}
          {msgLayouts.get('layouts.pagination.of')} {listData.last_page}
        </p>
      </div>
      <nav className="flex space-x-2">
        ...
        <a
          ...
          className={`pagination-btns ${
            listData.current_page !== 1 ? 'page-active' : 'page-active cursor-not-allowed'
          }`}
          ...
          .map((link) => (
            ...
            >
              {link.label === '&laquo; Previous'
                ? '« ' + msgLayouts.get('layouts.pagination.previous')
                : link.label === 'Next &raquo;'
                  ? msgLayouts.get('layouts.pagination.next') + ' »'
                  : link.label}
            </a>
          ))}
        ...
        <a
          ...
          className={`pagination-btns ${
            listData.next_page_url ? 'page-active' : 'page-active cursor-not-allowed'
          }`}
