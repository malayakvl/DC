...
//            dd($request->all());exit;
    ...
    public function findMaterial(Request $request) {
        return $this->withClinicSchema($request, function($clinicId) use ($request) {
            $name = $request->searchName;
            $resData = DB::table('materials')->select('materials.*', 'material_categories.percent as category_percent')
                ...
                ->groupBy('materials.name', 'materials.id', 'material_categories.percent')
                ->get();
